...
## 〇、★ V16 落死基线（2026-09-12 · 最新状态 · 先读这条）

> **项目现状的唯一权威描述** → `docs/00-统筹入口/10-当前基线-落死锚点-V16.md`
> **接手第一条命令** → `cd dsh-director-plugin && node scripts/baseline-check.mjs`（期望 `IS_PASS: TRUE（漂移=0）`）

| 项 | 值 |
|:---|:---|
| 主管道 | **官方 client 插件** `dsh-director-plugin/`（**不再是**改宿主编译产物 + apply.ps1；后者降为历史通道） |
| 产物 | `lib/client.js` **602,101 B / 45 模块**（React 平台外置） |
| 界面 | 宿主三页签【总监】【对话】【轨迹】（总监 `order:-1` 排最前）+ 三浮层（**设计图工作室** / 思维导图 / 总监弹窗） |
| 设计稿 | `docs/50-信息中心/V16-设计图·需求图·交互逻辑.html`（A 高保真 / B 需求图 / C 交互逻辑 / **D 设计图工作室**） |
| 新增能力 | **设计图工作室**：铺满全屏 · 拖拽增删 · 左侧交互逻辑七元组 · 底部"只处理本图"的修订对话 · 18 类元素 + 20 元素标准框架 |
| 落死机制 | `BASELINE.lock.json`（51 文件 md5）+ `scripts/baseline-check.mjs`（`--write` 重新封存，含正负对照校准） |
| 映射机制 | `scripts/gen-source-map.mjs` → 47 个源文件顶部 `@map` 块 + `dsh-director-plugin/docs/12-源码映射索引.md` |
| 闸门 | 静态 `TRUE` · CDP 模板 `TRUE` · 设计图纯函数 **49/49** · 工作室逐交互 **45/45** · 指纹 `漂移=0` |
| 纪律 | **不新开版本号**（用户原话「把这一版落死 / 我不想再开一版本了」）⇒ 基线演进就地更新锚点文档 + `--write` |

### 本轮（V16）修复的 14 处缺陷（分两类，类名本身是教训）

- **逻辑类 9 处**（会静默出错）：`dshLog` 未 import 且 catch 内也调它 ⇒ 连失败原因都拿不到 · `open=true && doc=null` 首帧空解引用 ⇒ 无错误边界卸载整棵 Shell 根且**每次重启自锁死** ·
  元素坐标写在 props 顶层而非 style ⇒ 拖拽/缩放/方向键"四连带全坏" · `ds-apply` 误绑 `onCommit` ⇒ 静默无反应（另一入口掩盖） ·
  `STEP=20` 魔法系数 ⇒ 「左 30」跑成 600px · 多词 label 指称不到 · **非 1 缩放下拖拽位移未 ÷k** · 文档称双写实则单写 · 标准框架七元组 0/20 完整
- **视觉类 5 处**（**逻辑断言全绿却漏过，只能靠截图发现**）：画布网格被不透明底层**盖死** · toast 压住底栏输入框 · toast **永不自动消失** ·
  20 个元素同色糊成一片 · 标准框架打开即右侧被裁

> ⚠️ **类名即教训**：7.2 那 5 条全是在"逻辑闸门全绿"状态下靠**截图 + DOM 计算样式回读**才抓到的。
> 故纪律第 5 条：**视觉改动必须截图复验**。
