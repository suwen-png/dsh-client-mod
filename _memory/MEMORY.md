...
### 14.7 🔴 当前最大阻断（须用户决策后解锁）

| 编号 | 阻断项 | 影响面 |
|:-----|:-------|:-------|
| **T-COG-001** | `D:\hermes-data\dsh-director` **空目录（0 文件、无 .git）**，插件 TS 源码工程缺失 | **阻断 T-V9D-004~010 与 T-PLUG-005~009 全系**（所有插件侧开发） |

- 未受影响：宿主补丁链路（`workspace/` → `apply.ps1`）可正常继续。
- 恢复源候选（未验证可用性）：git 基线 `12907c6` / 本仓 `snapshots/` / `D:\workspace\starfield-agent-history\src.backup-014`。
- **处置前提**：须先由用户确认该目录清空是否为有意迁移/归档。**确认前不得基于插件继续迭代**。