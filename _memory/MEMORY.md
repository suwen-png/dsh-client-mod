...
### 14.4 已知文档漂移（勘察发现，待整改）

1. `docs/30-开发链路/dsh-director插件开发环境文档.md` 引用的 `scripts/deploy.py` **不存在**（只有 deploy.ps1）
2. `INDEX.md` 字节数（1,423,779）/ 文档数（75）滞后于实测
3. `README.md` 目标包表头写 8 个但只列 7 行（漏 `dsh-client-ui-trajectory`）
4. `config/project_overview.md` 里程碑停在 V10 进行中；`config/agents.json` 含「测试技能X」调试残留
5. git 工作区 3 改 + 6 未跟踪（含 V12 设计稿 124KB 未纳管）
