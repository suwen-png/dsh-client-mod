...
> | 项 | 值 |
> |:---|:---|
> | 产物 | `lib/client.js` · **1,810,801 B** · **84 模块** · 绑定对账 **706 = 706**（251 条 import）· 内容指纹 **`8065109aaada40a9`** · React 外置零打包 · `inject = ["slots","sessions"]` |
> | 源码 | `src/**` **86 个 js · 30,788 行**，**每个文件带 `@map` 映射头** |
> | 映射索引 | [`docs/12-源码映射索引.md`](./docs/12-源码映射索引.md)（生成器 `scripts/gen-source-map.mjs`，86 文件） |
...
> | 本轮新增 | **第 20 批（2026-09-16）** —— **清除全部对话 + 继续跑流程**：走宿主官方通道 **`workspace.archiveSession`** 逐条下架 **115 条幽灵会话**（磁盘三口径早已 0，它们只在**主进程内存**里）⇒ 侧栏 `sessionRow` **6 → 0 行**，新增工具 `scripts/purge-host-sessions.mjs` · 修掉 **第 4 个真实产品缺陷**（派发存活集**必须排除已归档会话**：`branch-tree.archivedSessionIds()` + 报数 `archivedExcluded` / `archivedKnown`）· **闸门起点自举**改按 `sessionRow` class 认会话行（空壳会话无时间标记 ⇒ 原判据两段都不点 ⇒ 20 条假红）· 两条新环境事实：宿主 UI 状态**第四落点 = `localStorage`**（`groupExpansion[wsId]=false` ⇒ 侧栏不渲染会话行）+ **`blank` 草稿会话不挂 `conversation.view` slot**（`dp-root` 只在**真实会话**下出现）· 构建 **1,810,801 B / 706 = 706**　｜　上一批 **第 19 批（2026-09-16）** —— **会话复用 / 会话档案 / 清理 / 产物指纹守卫**：派发前**先复用已有会话**（真机 `reused=8 / created=0`，宿主会话**净增 0**）· 每会话**总监档案** `__dshDossier` · 会话**三落点清理** `scripts/clean-sessions.mjs` · 产物**内容指纹**（`build-stamp` + `check-stale-build` 装机前拦陈旧构建 · 真机 `window.__dshBuildStamp` 回读）· 修掉 **1 个真实产品缺陷**（宿主失败原因被丢弃）+ **2 处闸门缺陷**；闸门 **90→94** · 离线 **54 套**　｜　上一批 **第 17–18 批（2026-09-16）** —— **总监枢纽闭环（E1–E9）**：派发（`director-dispatch` + `dispatch-log` 台账）· 回收（`director-collect` 五态 + 总裁定）· **导图可见**（`data-dispatch`/`data-branch-state`/`data-said` 徽标 + 「📥 回收」）· 会话读取 `session-io`（走 `history({})` 事件流、**不切页签**）；修掉 **2 个真实产品缺陷**（台账 `applied` 翻倍 · `projectOf` 吞中文标点）；闸门 **69→90** · 4 个新模块单测 **53 条** · 离线 **51 套**　｜　上一批 **批次 15** —— 总监侧输入经**五步处理真正投递到原生对话**（`data-deliver-mode` 四态 · 三通道 `host-send`/`direct`/`open-then-send` · 宿主侧第三方送达凭据 `__directChatProbe`）· **分支链路聚焦**（默认只看本分支 / 「含上一层」/ 向下游下钻）· **导图总览弹窗**（左已完成 / 右待完成 · 分文件夹 · 可点选 · 可发修正）· **右下角两颗按钮「点不中」根因修复**（提示位改常驻等高槽，消除 23px 布局漂移）· 文案与命名整治 · **闸门纠错 5 处 + 导图零跳过** · **批次 12** —— 总监页背景采用原软件的背景（六令牌桥接宿主 `--dsw-alias-*`，与原生页签**同源同值** + 正负对照）· **批次 11** —— 单框展开/折叠与可拖动 · 点框右侧展开对话（首块＝「现在在做的事」）· 四处共用的**个性化设定**面板 · **四维流转** |
> | 基线闸门 | 静态 `TRUE`（阻塞 0 / 提示 17） · CDP 模板 `TRUE` · **平台桩唯一性 `lint-platform-stub` 9/9** · bundle 桩执行 **97/97** · 设计图纯函数 **49/49** · 版本层 **63/63** · 思维导图纯函数 **91/91** · 个性化+四维流转 **87/87** · 分支聚焦 **26/26** · 统筹编排 **33/33** · 设计稿卫生 **49/49** · 数据安全 **42/42** · 批次 1 **97/97**（2026-09-16 第十八轮实跑校正） · 批次 6 **66/66** · 批次 7 **66/66** · 批次 8 **93/93** · **弹窗离线 `verify-dialog` 268/268** · 反证 `prove-dup-gate` 红/绿双证 · 第 16 批验收 `verify-novel-split` **94/94 零跳过**（第十九轮：复用四条 `NS-11a–11d` 含核心验收 `NS-11c`，`NS-9f` 扩为三条合法出路） · 离线全量 **54 套 / PASS=42 / FAIL=0 / 真机跳过 12** · 构建 **1,810,801 B / 84 模块 · 706 = 706（251 import）** · 指纹 **90 文件 / 3,518,091 B · `漂移=0`** |
...
**九层验证（合计 536 项，离线全绿）**：源码级 **96/96** ｜ bundle 级 **63/63** ｜ 安装链路级 **60/60** ｜ 三层结构 **65/65** ｜ 自动同步 **66/66** ｜ 总监逻辑 **92/92** ｜ 数据安全 **42/42** ｜ **干净目录部署 52/52** ｜ 真机契约 **39/39**。另有 **真机逐交互点击验证 66/66（批次 8 时点值；现行 74/74）**（`scripts/cdp-click.mjs`，22 小节覆盖全部 30 个可交互元素，每项「点击 → 回读实际状态 → 比对预期」）。产物 `lib/client.js` **279,841 B / 32 模块**，React 零打包。
...
│   ├── cdp-click.mjs         ← ★ 真机逐交互点击验证（74 项 · 含职责配置快照/复原）
...
node scripts/cdp-click.mjs              # ＋ 真机逐交互点击验证               74 项
