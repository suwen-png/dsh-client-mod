...
function emit() {
	...
	p.push(`\t\t\ttry { trace.push("hostctx:" + typeof __entry.installHostCtxApi); __entry.installHostCtxApi(ctx); trace.push("hostctx:ok"); }`);
	p.push(`\t\t\tcatch (e) { trace.push("hostctx:ERR " + ((e && e.message) || e)); }`);
	...
	p.push(`\t\t\ttry { trace.push("inherit:" + typeof __entry.installDirectorInheritApi); __entry.installDirectorInheritApi(); trace.push("inherit:ok"); }`);
	p.push(`\t\t\tcatch (e) { trace.push("inherit:ERR " + ((e && e.message) || e)); }`);
	p.push(`\t\t\ttry {`);
	...
	p.push(`\t\t};`);
		...
		p.push(`\t\t// 🔴 第 42 轮：需求 4「所有文件夹都要有总监弹窗」必须读工作区**真实名**`);
		p.push(`\t\t//    （宿主侧栏显示的是 workspace.title），因此必须 inject workspaces ——`);
		p.push(`\t\t//    真机实测铁证：ctx.get("workspaces") 拿到的对象**没有 list 方法**`);
		p.push(`\t\t//    （diag: hasService=true / viaGet=true / "ctx.workspaces.list 不是函数"）`);
		p.push(`\t\t//    ⇒ 那不是该服务，只有 inject 声明后 ctx.workspaces 才是`);
		p.push(`\t\t//    WorkspaceRuntime（带 list()）—— 即**不能靠 ctx.get 兜底**。`);
		p.push(`\t\t// ⚠️ 曾一度怀疑「多声明 workspaces 会让插件推迟 apply ⇒ 插件 tab 不出现」，`);
		p.push(`\t\t//    后经对照实验证伪：那是**残留实例占单实例锁**造成的假红（环境问题，`);
		p.push(`\t\t//    10 个 Harness 进程未退干净 ⇒ 新实例起不来 ⇒ 连到旧页面 ⇒ 环里只有宿主 tab）。`);
		p.push(`\t\t//    干净起点下三服务 inject 的 tab 环正常（_ensure-page 恢复成功）。`);
		p.push(`\t\tvar inject = ["slots", "sessions", "workspaces"];`);
