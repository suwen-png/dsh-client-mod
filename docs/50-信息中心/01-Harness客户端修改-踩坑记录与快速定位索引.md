...
> 🔴 **本段已过时（`T-DOC-001 A10` 收口 · 2026-09-18 · 加横幅而非删改）**
> 本段以「**修改 `workspace/**` 宿主**」为主语成文于 2026-08-25。**此后主管道已迁移**：
> - **现行主管道 = 插件通道** `dsh-director-plugin/`（改 `src/**` → `gen-source-map` / `gen-key-files`
>   → lint 七把 → `build/build.mjs` → `plugin-install --apply` → **重启 Harness**），
>   全链见仓库根 `AGENTS.md` 与锚点 `docs/00-统筹入口/10-…-落死锚点-V16.md` §四。
> - **改宿主 `workspace/**` 属退坡项**，须**用户显式授权**（`T-PLUG-005-B6`）；
>   ⚠️ `workspace/**` 被 `.gitignore` 排除 ⇒ **git 无法回滚**，唯一回滚点是
>   `snapshots/host-b6-20260911-before-patch`（不可删）。
> - 本段以下内容保留为**历史参考**（当时的操作路径与清缓存手法仍有考古价值）；
>   **凡与插件通道冲突，一律以插件通道为准。**
