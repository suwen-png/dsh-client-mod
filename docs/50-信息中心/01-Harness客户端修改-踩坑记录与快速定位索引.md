...
**现象**：`lint-cdp-templates.mjs` 报「第 508 行模板闭合处异常 → 后继 "[d"」。

**真因**：注释写在 `evalExpr(\`...\`)` 的**模板字符串内部**，且用了反引号（如 \`DirectorPage\`）—— 反引号提前闭合模板，后面的 JS 变成裸字符串拼接。

**正确做法**：① 这类注释**写在模板字符串之外**；② 必须留在模板内时，不要用反引号（用「」或直接写裸词）；③ 改完必跑 `node scripts/lint-cdp-templates.mjs`。

**来源**：实测 2026-09-13（`cdp-click-dialog.mjs` D7 / D14 两处）。
